...
CHANGE_MANIFEST_ENV = "AUTORESEARCH_CHANGE_MANIFEST"
...
def run_once(config: AutoresearchConfig) -> int:
    ...
    try:
        entry["memory_save"] = save_to_memory(config, entry)
    except (OSError, URLError, TimeoutError) as exc:
        entry["memory_save"] = {"status": "failed", "detail": str(exc)[:300]}
        entry["status"] = "checks_failed"
        entry["lesson"] = "Required RPA Memory save failed."
        append_jsonl(config.jsonl_path, entry)
        print(json.dumps(redact_value(entry), indent=2))
        return 1
    append_jsonl(config.jsonl_path, entry)
...
def build_run_entry(
    ...
    if change_manifest:
        entry["change_manifest"] = normalize_change_manifest(change_manifest)
        entry["change_attribution"] = attribute_change_manifest(
            manifest=entry["change_manifest"],
            previous=previous,
            status=status,
            metric=primary_metric,
            metrics=metrics,
...
def load_change_manifest() -> dict[str, Any] | None:
    raw = os.environ.get(CHANGE_MANIFEST_ENV, "").strip()
    if not raw:
        return None
    try:
    ...
    if not isinstance(parsed, dict):
        return {
            "schema": "ahe.change_manifest.v1",
            "manifest_error": "not_an_object",
        }
    return parsed


def normalize_change_manifest(manifest: dict[str, Any]) -> dict[str, Any]:
    normalized: dict[str, Any] = {
        "schema": str(manifest.get("schema") or "ahe.change_manifest.v1")[:80],
        "source": _manifest_text(manifest.get("source") or "unknown"),
    }
    for key in (
        "id",
        "hypothesis",
        ...
        if key in manifest and manifest.get(key) is not None:
            normalized[key] = _manifest_text(manifest.get(key))
    for key in (
        "changed_files",
        "failure_evidence",
        "predicted_fixes",
        ...
        if key in manifest:
            normalized[key] = _manifest_list(manifest.get(key))
    metadata = manifest.get("metadata")
    if isinstance(metadata, dict):
        normalized["metadata"] = {
            str(key)[:80]: _manifest_text(value)
            for key, value in list(metadata.items())[:MAX_MANIFEST_ITEMS]
        }
    return normalized
...
def _manifest_text(value: Any) -> str:
    ...
    return text[:MAX_MANIFEST_TEXT]


def _manifest_list(value: Any) -> list[str]:
    if not isinstance(value, list):
        value = [value]
    return [_manifest_text(item) for item in value[:MAX_MANIFEST_ITEMS] if item is not None]


def attribute_change_manifest(
    ...
    previous_best = best_kept_metric(previous, direction, metric_name=metric_name)
    metric_delta = None
    improved = status == "keep"
    if metric is not None and previous_best is not None:
        metric_delta = metric - previous_best
        improved = metric_delta < 0 if direction == "lower" else metric_delta > 0
    predicted_fixes = manifest.get("predicted_fixes") or []
    predicted_regressions = manifest.get("predicted_regressions") or []
    per_target = per_target_attribution(
        previous=previous,
        current_metrics=metrics,
        predicted_fixes=predicted_fixes,
        predicted_regressions=predicted_regressions,
    )
    mode = "per_target_and_aggregate" if per_target else "aggregate_metric_delta"
    fix_supported = bool(predicted_fixes) and (
        improved or bool((per_target or {}).get("fix_true_positives"))
    )
    regression_supported = bool(predicted_regressions) and (
        status in {"discard", "crash", "checks_failed"}
        or bool((per_target or {}).get("regression_true_positives"))
    )
    return {
        "schema": "ahe.change_attribution.v1",
        "mode": mode,
        "metric_name": metric_name,
        "previous_best_metric": previous_best,
        "current_metric": metric,
        "metric_delta": metric_delta,
        "outcome": status,
        "prediction_summary": {
            "predicted_fix_count": len(predicted_fixes),
            "predicted_regression_count": len(predicted_regressions),
            "fix_prediction_supported": fix_supported,
            "regression_prediction_supported": regression_supported,
        },
        **({"per_target": per_target} if per_target else {}),
    }
...
def per_target_attribution(
    ...
    previous_metrics = previous_kept_metrics(previous)
    if not previous_metrics:
        return None
    current_targets = target_metrics(current_metrics)
    previous_targets = target_metrics(previous_metrics)
    shared_targets = sorted(set(current_targets) & set(previous_targets))
    ...
    for target in shared_targets:
        current = current_targets[target]
        old = previous_targets[target]
        if current == old:
    ...
    fix_matches = matched_predictions(predicted_fixes, improved_targets)
    regression_matches = matched_predictions(predicted_regressions, regressed_targets)
    return {
        "target_metric_prefixes": ["task.", "target.", "workflow.", "case."],
        "observed_improved": improved_targets,
        "observed_regressed": regressed_targets,
        "observed_unchanged": unchanged_targets,
        "fix_true_positives": fix_matches,
        "regression_true_positives": regression_matches,
        "fix_precision": ratio(len(fix_matches), len(predicted_fixes)),
        "fix_recall": ratio(len(fix_matches), len(improved_targets)),
        "regression_precision": ratio(len(regression_matches), len(predicted_regressions)),
        "regression_recall": ratio(len(regression_matches), len(regressed_targets)),
    }


def previous_kept_metrics(previous: list[dict[str, Any]]) -> dict[str, float]:
...
def target_metrics(metrics: dict[str, float]) -> dict[str, float]:
    prefixes = ("task.", "target.", "workflow.", "case.")
    targets: dict[str, float] = {}
    for name, value in metrics.items():
        for prefix in prefixes:
            if name.startswith(prefix):
                targets[name[len(prefix):]] = value
                break
    return targets


def matched_predictions(predictions: list[str], observed_targets: list[str]) -> list[str]:
    matches = []
    normalized_observed = {target: normalize_target(target) for target in observed_targets}
    for prediction in predictions:
        normalized_prediction = normalize_target(prediction)
        if not normalized_prediction:
            continue
        for target, normalized_target in normalized_observed.items():
            if normalized_target and (
                normalized_target in normalized_prediction
                or normalized_prediction in normalized_target
            ):
...
def save_to_memory(config: AutoresearchConfig, entry: dict[str, Any]) -> dict[str, str]:
    ...
    payload = {
        ...
        "metadata": {
            ...
            "component_type": (entry.get("change_manifest") or {}).get("component_type", ""),
            "change_attribution_mode": (entry.get("change_attribution") or {}).get("mode", ""),
        },
    }
    request = Request(
        url,
    ...
    series = [
        {
            ...
            "component_type": (entry.get("change_manifest") or {}).get("component_type", ""),
            "change_attribution": entry.get("change_attribution"),
        }
        for entry in entries
    ]
    implementations = [
        {
            ...
            "component_type": (entry.get("change_manifest") or {}).get("component_type", ""),
            "changed_files": (entry.get("change_manifest") or {}).get("changed_files", []),
            "change_attribution": entry.get("change_attribution"),
        }
...
<body>
    ...
    <section class="grid">
      ...
      <div class="panel full">
        ...
        <div style="overflow:auto">
          <table>
            <thead><tr><th>Run</th><th>Status</th><th>Metric</th><th>Checks</th><th>Component</th><th>Attribution</th><th>Lesson</th><th>Time</th></tr></thead>
            <tbody id="runs"></tbody>
          </table>
        </div>
      </div>
  ...
  <script>
    ...
    function attributionSummary(run) {{
      const attribution = run.change_attribution || {{}};
      const summary = attribution.prediction_summary || {{}};
      if (!attribution.mode) return "-";
      const fix = summary.fix_prediction_supported ? "fix✓" : "fix?";
      const regression = summary.regression_prediction_supported ? "reg✓" : "reg?";
      return `${{fix}} ${{regression}} Δ=${{fmt(attribution.metric_delta)}}`;
    }}
    function render(data) {{
      ...
      document.getElementById("runs").innerHTML = [...(data.series || [])].reverse().map(run => `
        <tr>
          ...
          <td>${{escapeHtml(run.component_type || "")}}</td>
          <td>${{escapeHtml(attributionSummary(run))}}</td>
...
- Before stopping, make the change falsifiable: identify failure evidence, root cause, targeted fix, predicted fixes, predicted regressions, touched component type, and rollback hint in your final note.
