...
In the free profile, automatic merge is allowed for repository-scope changes that improve the primary metric, pass correctness checks, contain no forbidden generated/credential/local artifacts, pass review when `require_review=true`, and preserve a clean post-merge working tree.
...
The supervisor is configured by `.autoresearch/autoresearch.supervisor.json`. The default profile is free repository-scope mutation; `.autoresearch/autoresearch.sovereign.json` carries the same unattended 24/7 profile. The default interval is one hour. Worktrees live under `.autoresearch/worktrees/` and are ignored by git.

The default free-profile scout subagents are `code_explorer`, `failure_analyst`, `speed_optimizer`, and `tech_radar_adopter`. They run through Codex CLI with a read-only sandbox, produce JSON candidates only, and cannot bypass the benchmark, integration gate, optional review gate, post-merge checks, or push gate.
...
- No unverifiable broad rewrites; free mode may make a coherent batch, but the batch must stay testable.
...
The autoresearch before-hook now runs `tools/tech_radar.py` by default. The radar watches `.autoresearch/tech_radar.sources.json`, stores runtime state in ignored `.autoresearch/tech_radar.*` artifacts, and emits candidate ideas when an authoritative source changes.

The radar is intentionally read-only. It creates evidence and candidate ideas; the free supervisor still owns implementation, deterministic checks, optional review, merge, rollback, and push decisions.
