...
## 2026-06-22 — Patch proposer JSON read slice

Deleted:
- Patch proposer direct `json.loads(Path(...).read_text())` report read

Combined:
- Patch proposer report loading now uses the core required JSON reader

Source of truth:
- `harness/core/artifacts.py` for required JSON artifact reads

Checks:
- `.venv\Scripts\python.exe tools\propose_patch.py --help` → passed
